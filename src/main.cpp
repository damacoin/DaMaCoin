...
uint256 hashGenesisBlock("0x1ae87095033f2bc27c31c0614465b64ebb08b7174aa589f20e63078e8538e7ea");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // DaMaCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "DaMaCoin Signed Message:\n";
...
{
    // DaMaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 10 * 60; // DaMaCoin: 10 minutes
static const int64 nTargetSpacing = 60; // DaMaCoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // DaMaCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // DaMaCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1ae87095033f2bc27c31c0614465b64ebb08b7174aa589f20e63078e8538e7ea");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        const char* pszTimestamp = "DaMaCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 88 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04fbb2a7268deca959d08a377f43f384c2de48cb1e434d61b18d8536f07d1ad81983beb3cdd10f55d105b8baec264e13bf7afd72bd6cee424c055ad15a84d7dd78") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1393790400;
        ...
        assert(block.hashMerkleRoot == uint256("0x0451c843be144f2e5a5ad80bf0738a3b4ca71153e2ab18402b548cdf8c4aa026"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // DaMaCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DamacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DamacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DamacoinMiner(CWallet *pwallet)
{
    printf("DamacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("damacoin-miner");
    ...
    try { loop {
        ...
        printf("Running DamacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DamacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DamacoinMiner, pwallet));
}
