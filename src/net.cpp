...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "DaMaCoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"seed1.damacoin.org", "seed1.damacoin.org"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. DaMaCoin is probably already running."), addrBind.ToString().c_str());
        else
